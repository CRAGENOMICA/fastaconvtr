    ...
    *nsam = nsamtot;
    ...
    else
    {
        ...
        if (args->file_mask_char[0] == '\0')
        {
            ...
            if (args->file_out[0] == '\0')
                strcpy(mask_name, args->file_in);
            else
                strcpy(mask_name, args->file_out);
            if (args->npops == 0)
                args->npops = 1;
            ...
            if (args->include_unknown)
                strcat(mask_name, "_IncludeMissingVariantsmhits");
            else
                strcat(mask_name, "_ExcludeMissingVariantsmhits");
            if (args->outgroup == 0)
                strcat(mask_name, "_NOoutg");
            if (args->outgroup == 1)
                strcat(mask_name, "_outg");
            if (args->ploidy == 1)
                strcat(mask_name, "_ploidy1");
            if (args->ploidy >= 2)
                strcat(mask_name, "_ploidy2");
        ...
        for (x = 0; x < nsamtot / args->ploidy; x++)
        {
            for (xx = 0; xx < n_site; xx++)
            {
                ...
                if (w >= 48 + 5)
                    bzprintf(file_mask, file_mask_gz, "0"); /*missing*/
                else
                    bzprintf(file_mask, file_mask_gz, "1"); /*normal*/
            }
            bzprintf(file_mask, file_mask_gz, "\n");
            if (args->ploidy >= 2)
            {
        ...
        */
        if (args->gfffiles == 1)
        {
            if (args->file_weights_char[0] == '\0')
            {
                ...
                strcat(file_weights_char, args->criteria_transcript);
                if (args->include_unknown)
                    strcat(file_weights_char, "_IncludeMissingmhits");
                else
                    strcat(file_weights_char, "_ExcludeMissingmhits");
                if (args->outgroup == 0)
                    strcat(file_weights_char, "_NOoutg");
                if (args->outgroup == 1)
                    strcat(file_weights_char, "_outg");
    ...
    unsigned long first)
{
    ...
    if (file_ws != 0)
    {
        ...
        else
        {
            ...
            {
                while (strcmp(chr_name, valn) != 0)
                { /*exclude those rows having different scaffold than chr_name*/
                    if (inside_chr == 1)
                    {
                        inside_chr = -1; /*we passed target region*/
                        break;
